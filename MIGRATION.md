...
If you keep an existing reverse proxy, point it to the unified `kaneo` service on that port.
Do not configure separate `api` and `web` upstreams.
...
For another reverse proxy, expose the Kaneo container port locally and proxy the public URL to `http://localhost:5173`.
